...
function removeFirstOccurrences(str, value) {
  ...
  if (firstIndex >= 0) return str.slice(0, firstIndex) + str.slice(lastIndex);
  return str;
}
...
function isPalindrome(str) {
  const myStr = str.toLowerCase().replace(/\W/g, '').split('');
  if (myStr.join('') === myStr.reverse().join('')) return true;
  return false;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < arrStr.length; i += 1) {
    indexDecode = abc.indexOf(arrStr[i]);
    newStr.push(decode.split('')[indexDecode]);
  }
  return newStr.join('');
}
...
function getCardId(value) {
  ...
  const index = arr.findIndex((card) => {
    return card === value;
  });
  return index;
}
